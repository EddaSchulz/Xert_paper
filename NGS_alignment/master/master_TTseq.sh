#!/bin/bash
# Master script to align TT-seq data and create coverage tracks
fastq_dir=''
work_dir=$(pwd)'/'
path=''

help() {
   echo "Builds masked mm10 genome, aligns and processes TT-seq data and creates coverage tracks."
   echo
   echo "Syntax: ./master_TTseq.sh [-d|f|p|h]"
   echo "options:"
...
if [[ $fastq_dir == '' ]]
then
	echo -e "Please provide a path to TT-seq FASTQ files (GSE167356) with -f"
  exit 1
fi
