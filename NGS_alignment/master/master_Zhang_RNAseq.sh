#!/bin/bash
# Master script to align embryo RNA-seq data from Zhang et al. 2018 (GSE76505)
fastq_dir=''
work_dir=$(pwd)'/'
path=''

help() {
   echo "Builds mm10 genome, aligns and processes embryo RNA-seq data from Zhang et al. 2018."
   echo
   echo "Syntax: ./master_Zhang_RNAseq.sh [-d|f|p|h]"
   echo "options:"
   echo "d     Provide working directory. [optional]"
   echo "f     Provide directory containing FASTQ files (GSE76505). [mandatory]"
...
if [[ $fastq_dir == '' ]]
then
	echo -e "Please provide a path to embryo RNA-seq FASTQ files from Zhang et al. 2018 (GSE76505) with -f"
  exit 1
fi
