#!/bin/bash
# Master script to align CUT&Tag data, call peaks and create coverage tracks
fastq_dir=''
work_dir=$(pwd)'/'
path=''

help() {
   echo "Builds masked mm10 genome, aligns and processes CUT&Tag data, Calls peaks and creates coverage tracks."
   echo
   echo "Syntax: ./master_CUTnTag.sh [-d|f|p|h]"
   echo "options:"
   echo "d     Provide working directory. [optional]"
   echo "f     Provide directory containing FASTQ files (GSE167353). [mandatory]"
...
if [[ $fastq_dir == '' ]]
then
	echo -e "Please provide a path to CUT&Tag FASTQ files (GSE167353) with -f"
  exit 1
fi
...
${path}scripts/CUTnTag_peaks.sh $work_dir $bam_dir
